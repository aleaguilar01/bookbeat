...
const TestPage: FC<TestPageProps> = ({
    ...
    const fetchSearchResults = async () => {
      try {
        const response = await fetch(
          `http://localhost:3000/music/search?search=${encodeURIComponent(
            search
          )}`,
          {
            method: "GET",
        ...
        setSearchResults(
          data.tracks.items.map((track) => {
            const smallestAlbumImage = track.album.images.reduce<AlbumImage>(
  ...
  return (
    ...
    >
      ...
      <div>
        <Player
          accessToken={user.spotifyToken.access_token}
          trackUri={playingTrack?.uri || playingPlaylist?.uri}
        />
      </div>
    </Container>
  );
